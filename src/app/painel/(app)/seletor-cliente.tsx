...
export function SeletorCliente({
  ...
  return (
    <div className="rounded-md border border-zinc-200 bg-zinc-50 p-3">
      <p className="mb-2 text-xs font-medium text-brand-gray">{label}</p>
      ...
      {!novo ? (
        <>
          ...
          >
            <option value="">
              {obrigatorio ? "Selecione um cliente" : "Não informado"}
            </option>
            {clientes.map((c) => (
